...
<template>
  <Teleport to="body"><Transition name="hud-dialog"><div v-if="visible" class="dialog-backdrop" @mousedown.self="emit('close')">
    <section ref="dialog" class="cyber-dialog compare-dialog" role="dialog" aria-modal="true" aria-labelledby="compare-title" tabindex="-1" @keydown="onDialogKeydown">
      ...
      <div class="compare-scroll" tabindex="0" aria-label="结果差异"><table class="compare-table"><thead><tr><th scope="col">基线输出 · − 删除</th><th scope="col">本次输出 · + 新增</th></tr></thead><tbody><tr v-for="(row, index) in comparison.rows" :key="index" :class="`compare-row--${row.kind}`"><td :class="{ 'compare-cell--removed': row.kind === 'removed' }"><span class="compare-marker" aria-hidden="true">{{ row.kind === 'removed' ? '−' : ' ' }}</span><pre>{{ row.left ?? '' }}</pre></td><td :class="{ 'compare-cell--added': row.kind === 'added' }"><span class="compare-marker" aria-hidden="true">{{ row.kind === 'added' ? '+' : ' ' }}</span><pre>{{ row.right ?? '' }}</pre></td></tr></tbody></table></div>
